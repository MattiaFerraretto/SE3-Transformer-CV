...
from se3.utils.steerable import *
from se3.utils.wrappers import *
from se3.fibers import Fiber, fiber2head
...
class RadialFunc(nn.Module):
    ...
    def __init__(self, num_freq, in_dim, out_dim, edge_dim: int=0):
        ...
        self.net = nn.Sequential(nn.Linear(self.edge_dim+1,self.mid_dim),
                                #nn.Linear(self.edge_dim,self.mid_dim),
                                 BN(self.mid_dim),
                                 nn.ReLU(),
    ...
    def forward(self, x):
        #print('num_freq', self.num_freq)
        #print('in_dim', self.in_dim)
        #print('out_dim', self.out_dim)
        #print('edge_dim', self.edge_dim)
        y = self.net(x)
        #print('y', y)
...
class GConvSE3Partial(nn.Module):
    ...
    def forward(self, h, G=None, r=None, basis=None, **kwargs):
        ...
        with G.local_scope():
            ...
            #print('f_in', self.f_in.structure)
            #print('f_out', self.f_out.structure)

            for (mi, di) in self.f_in.structure:
                for (mo, do) in self.f_out.structure:
                    etype = f'({di},{do})'
                    #print(etype)
                    #print(self.kernel_unary[etype])
...
class GMABSE3(nn.Module):
    ...
    def forward(self, v, k: Dict=None, q: Dict=None, G=None, **kwargs):
        ...
        Returns:
            tensor with new features [B, n_points, n_features_out]
        """
        #print('Attention f_value:', self.f_value)
        with G.local_scope():
            # Add node features to local graph scope
